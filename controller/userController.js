...
exports.transferToOtherBanks = async (req, res) => {
  const { receiverAccountNumber, receiverBankCode, amount } = req.body;
  const senderId = req.user.id; // Assuming the sender's ID is available in the authenticated user's object
  ...
  try {
    ...
    // Initiate transfer with Paystack API
    const transferData = {
      source: 'balance',
      amount: amount * 100, // Convert amount to kobo (e.g., Naira to kobo)
      recipient: receiverAccountNumber,
      bank_code: receiverBankCode,
      currency: 'NGN', // Adjust according to your currency
    };
    ...
    if (transferResponse.status !== 200) {
      return res.status(500).json({ message: 'Transfer failed', error: transferResponse.data });
    }
    ...
    const transfer = new Transfer({
      ...
      recipientBankCode: receiverBankCode,
      amount,
      senderBalanceBefore,
      senderBalanceAfter,
    });
